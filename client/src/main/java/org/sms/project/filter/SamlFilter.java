...
public class SamlFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain)
        ...
        if (isIgnoreUrl(uri) || session.getAttribute(SysConstants.LOGIN_USER) != null) {
            ...
            if (ticket == null) {
                response.sendRedirect(SysConstants.IPDLOGIN_PAGE + SysConstants.METHOD_SPILT_CHAR
                        + SysConstants.REDIRECT_URL_KEY + SysConstants.PARAM_VALUE
                        + URLEncoder.encode(request.getRequestURL().toString(), SysConstants.CHARSET));
                return;
            }
            /**
             * 如果有票据，则验证票据
            ...
            response.sendRedirect(SysConstants.SEND_ARTIFACT_URI + SysConstants.METHOD_SPILT_CHAR
                    + URLEncoder.encode(request.getRequestURL().toString(), SysConstants.CHARSET));
        }
    }
